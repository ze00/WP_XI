    ...
    public/*internal*/ class Board : Widget, ButtonListener
    {
        ...
        public Board(LawnApp theApp)
        {
            ...
            else
            {
                if (true)
                {
                    mCheatButton = new GameButton(114514, this);
                    mCheatButton.mDrawStoneButton = true;
                    mCheatButton.Resize(Constants.UIProgressMeterPosition.X - Constants.Board_Offset_AspectRatio_Correction, Constants.UIProgressMeterPosition.Y, AtlasResources.IMAGE_FLAGMETER.GetCelWidth(), AtlasResources.IMAGE_FLAGMETER.GetCelHeight());
                }
        ...
        public void PostInitLevel()
        {
            ...
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_QYGH)
            {
            ...
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_BOSS)
            {
                ...
                mZombieCountDownStart = mZombieCountDown;
                AddPlant(1, 1, SeedType.Flowerpot, SeedType.None);
                AddPlant(1, 1, SeedType.Umbrella, SeedType.None);
                AddPlant(1, 4, SeedType.Flowerpot, SeedType.None);
                AddPlant(1, 4, SeedType.Umbrella, SeedType.None);
            }
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_ZHZJ)
            {
        ...
        public void InitLevel()
        {
            ...
            else if (mApp.IsLastStandLevel() && !mApp.IsAdventureMode())
            {
                mSunMoney = 5000;
            }
            ...
            else if (mLevel == ExtGameLevel.CUSTOM_XJZY)
            {
                mSunMoney = 150;
            }
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_ZHZJ)
            ...
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_T3W4 || mLevel == ExtGameLevel.CUSTOM_LEVEL_JJDCG)
            {
                mSunMoney = 100;
            }
            // 第六大关开局75阳光，4列花盆
            else if (mLevel >= 51 && mLevel < 60 || mLevel == ExtGameLevel.CUSTOM_LEVEL_MYPD)
            {
                mSunMoney = 125;
                // 6-10作为自选卡的僵王关
            }
            else if (mLevel == ExtGameLevel.CUSTOM_MINIGAME_65)
            {
                mSunMoney = 4000;
            }
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_BOSS)
            ...
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_CTCJ)
            {
                mSunMoney = 325;
            }
            else if (mLevel == ExtGameLevel.CUSTOM_LEVEL_LLFX)
            {
                mSunMoney = 400;
            }
            else
        ...
        public Plant AddPlantOriginal(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
        {
        ...
        {
            if (theSeedType == SeedType.Sprout && !HasConveyorBeltSeedBank())
            {
                int n;
                do
                {
                ...
                return AddPlantOriginal(theGridX, theGridY, SeedType.Imitater, mSeedBank.mSeedPackets[n].mPacketType);
            }
            // 只允许普通选卡中的土豆产生复制
            if (theSeedType == SeedType.Potatomine && theImitaterType == SeedType.None)
            {
                AddPlantOriginal(theGridX, theGridY, theSeedType, theImitaterType);
                int[] rows = new int[] { 0, 1, -1 };
                int[] cols = new int[] { 0, 1, -1 };
                foreach (int row in rows)
                {
                    foreach (int col in cols)
                    {
                        if (CanPlantAt(theGridX + col, theGridY + row, SeedType.Potatomine) == PlantingReason.Ok)
                        {
                            return AddPlantOriginal(theGridX + col, theGridY + row, SeedType.Imitater, SeedType.Potatomine);
                        }
                    }
                }
            }
            return AddPlantOriginal(theGridX, theGridY, theSeedType, theImitaterType);
        }
        ...
        public ZombieType PickGraveRisingZombieType(int theZombiePoints)
        {
            ...
            if (mLevel == ExtGameLevel.CUSTOM_LEVEL_MYPD)
            {
                num = 7;
                aZombieWeightArray[0].mItem = ZombieType.Normal;
                aZombieWeightArray[0].mWeight = 20;
                aZombieWeightArray[1].mItem = ZombieType.TrafficCone;
                aZombieWeightArray[1].mWeight = 20;
                aZombieWeightArray[2].mItem = ZombieType.Pail;
                aZombieWeightArray[2].mWeight = 20;
                aZombieWeightArray[3].mItem = ZombieType.Newspaper;
                aZombieWeightArray[3].mWeight = 10;
                aZombieWeightArray[4].mItem = ZombieType.Door;
                aZombieWeightArray[4].mWeight = 10;
                aZombieWeightArray[5].mItem = ZombieType.Football;
                aZombieWeightArray[5].mWeight = 10;
                aZombieWeightArray[6].mItem = ZombieType.Zamboni;
                aZombieWeightArray[6].mWeight = 10;
            }
        ...
        public ZombieType PickZombieType(int theZombiePoints, int theWaveIndex, ZombiePicker theZombiePicker)
        {
            ...
            for (ZombieType i = 0; i < ZombieType.ZombieTypesCount; i++)
            {
                ...
                if (mZombieAllowed[(int)i])
                {
                    ...
                    {
                        if (ExtGameDef.CUSTOM_MINIGAME_55_ZOMBIE_WEIGHTS_OVERRIDE.TryGetValue((ZombieType)i, out aPickWeight))
                        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // 水草种在屋顶的判定
                            if (theType == SeedType.Tanglekelp && StageHasRoof() && plantsOnLawn.mNormalPlant == null)
                            {
                                return PlantingReason.Ok;
                            }
                        ...
                        {
                            if ((theType == SeedType.Spikeweed || theType == SeedType.Spikerock) && (mGridSquareType[theGridX, theGridY] == GridSquareType.Pool || (plantsOnLawn.mUnderPlant != null && plantsOnLawn.mUnderPlant.mSeedType != SeedType.Flowerpot)))
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if (theType == SeedType.Sprout && plantsOnLawn.mNormalPlant == null)
                                        return PlantingReason.Ok;
        ...
        public override void MouseDown(int x, int y, int theClickCount)
        {
            ...
            else if (mStoreButton != null && mStoreButton.IsMouseOver() && CanInteractWithBoardButtons() && theClickCount > 0)
            {
                ...
                else if (mApp.IsLastStandLevel() || mApp.mGameMode == GameMode.Upsell)
                {
                    mApp.PlaySample(Resources.SOUND_GRAVEBUTTON);
                }
            }
        ...
        public override void MouseUp(int x, int y, int theClickCount)
        {
            ...
            else if ((mCheatButton?.IsMouseOver() ?? false) && CanInteractWithBoardButtons() && theClickCount > 0 && mApp.mDebugKeysEnabled)
            {
                RefreshSeedPacketFromCursor();
                if (mApp.GetDialog(Dialogs.DIALOG_GAME_OVER) != null || mApp.GetDialog(Dialogs.DIALOG_LEVEL_COMPLETE) != null)
                {
        ...
        public int GetNumSeedsInBank()
        {
            ...
            if (mApp.IsAdventureMode() && mLevel == ExtGameLevel.CUSTOM_MINIGAME_65)
            {
                return 5;
            }
        ...
        public void MouseUpWithPlant(int x, int y, int theClickCount)
        {
            ...
            int theGridX = PlantingPixelToGridX((int)(x * Constants.IS), (int)(y * Constants.IS), seedTypeInCursor);
            int theGridY = PlantingPixelToGridY((int)(x * Constants.IS), (int)(y * Constants.IS), seedTypeInCursor);
            if (theGridX < 0 || theGridX >= Constants.GRIDSIZEX || theGridY < 0 || theGridY >= Constants.MAX_GRIDSIZEY)
            {
                RefreshSeedPacketFromCursor();
                mApp.PlayFoley(FoleyType.Drop);
            ...
            PlantingReason plantingReason = CanPlantAt(theGridX, theGridY, seedTypeInCursor);
            if (plantingReason != PlantingReason.Ok)
            {
                ...
                {
                    SeedType artChallengeSeed = mChallenge.GetArtChallengeSeed(theGridX, theGridY);
            ...
            Plant topPlantAt = GetTopPlantAt(theGridX, theGridY, TopPlant.OnlyNormalPosition);
            bool flag = false;
            ...
            {
                Plant topPlantAt2 = GetTopPlantAt(theGridX, theGridY, TopPlant.OnlyPumpkin);
            ...
            {
                Plant topPlantAt3 = GetTopPlantAt(theGridX + 1, theGridY, TopPlant.OnlyNormalPosition);
                if (topPlantAt3 != null)
                {
                    topPlantAt3.Die();
                }
            }
            if (seedTypeInCursor == SeedType.Cattail || seedTypeInCursor == SeedType.Spikeweed || seedTypeInCursor == SeedType.Spikerock)
            {
                PlantsOnLawn plantsOnLawn = default(PlantsOnLawn);
                GetPlantsOnLawn(theGridX, theGridY, ref plantsOnLawn);
            ...
            if (mCursorObject.mCursorType == CursorType.PlantFromGlove)
            {
                ...
                if (mLevel == ExtGameLevel.CUSTOM_LEVEL_LLFX)
                {
                    ...
                    if (aTopPlantAtGrid != null)
                    {
                        DoPlantingEffects(theGridX, theGridY, aTopPlantAtGrid, false);
                        return;
                    }
                    DoPlantingEffects(theGridX, theGridY, thePlant, false);
                }
                else
                {
                    mApp.mZenGarden.MovePlant(thePlant, theGridX, theGridY);
                }
            }
            ...
            {
                AddPlant(theGridX, theGridY, mCursorObject.mType, mCursorObject.mImitaterType);
                Coin coin = mCoins[mCoins.IndexOf(mCursorObject.mCoinID)];
                mCursorObject.mCoinID = null;
                coin.Die();
            }
            ...
            {
                Plant plant = AddPlant(theGridX, theGridY, mCursorObject.mType, mCursorObject.mImitaterType);
                if (flag)
                {
            ...
            if (mApp.mGameMode == GameMode.ChallengeColumn)
            {
                for (int i = 0; i < Constants.MAX_GRIDSIZEY; i++)
                {
                    if (i != theGridY && CanPlantAt(theGridX, i, seedTypeInCursor) == PlantingReason.Ok)
                    {
                        if (seedTypeInCursor == SeedType.Wallnut || seedTypeInCursor == SeedType.Tallnut)
                        {
                            Plant topPlantAt4 = GetTopPlantAt(theGridX, i, TopPlant.OnlyNormalPosition);
                        ...
                        if (seedTypeInCursor == SeedType.Pumpkinshell)
                        {
                            Plant topPlantAt5 = GetTopPlantAt(theGridX, i, TopPlant.OnlyPumpkin);
                            if (topPlantAt5 != null && topPlantAt5.mSeedType == seedTypeInCursor)
                            {
                                topPlantAt5.Die();
                            }
                        }
                        AddPlant(theGridX, i, mCursorObject.mType, mCursorObject.mImitaterType);
                    }
                }
            }
            if (mTutorialState == TutorialState.Level1PlantPeashooter)
        ...
        public void MouseDownWithTool(int x, int y, int theClickCount, CursorType theCursorType, bool posScaled)
        {
            ...
            if (plant != null)
            {
                ...
                {
                    mCursorObject.mType = plant.mSeedType;
                    mCursorObject.mImitaterType = plant.mImitaterType;
                    mCursorObject.mCursorType = CursorType.PlantFromGlove;
                    mCursorObject.mGlovePlantID = plant;
        ...
        public void UpdateZombieSpawning()
        {
            ...
            if (mCurrentWave == mNumWaves)
            {
                ...
                //if (mLevel == ExtGameLevel.CUSTOM_XJZY)
                //{
                //    SeedPacket packet = Array.Find(mSeedBank.mSeedPackets, (SeedPacket packet) =>
                //    {
                //        return packet.mPacketType == SeedType.Gravebuster;
                //    });
                //    if (packet != null)
                //    {
                //        packet.mPacketType = SeedType.Chomper;
                //        DisplayAdviceAgain("YOU GOT A 0-COUNTDOWN CHOMPER NOW!", MessageStyle.HintFast, AdviceType.None);
                //        packet.mRefreshTime = 0;
                //        packet.mRefreshCounter = 0;
                //        packet.mRefreshing = false;
                //    }
                //}
            ...
            bool flag = mApp.IsWallnutBowlingLevel() || mApp.IsLastStandLevel();
            if (mCurrentWave == mNumWaves && mApp.IsSurvivalMode())
            {
            ...
            else
            {
                ...
                {
                    mZombieHealthToNextWave = (int)(0.33f * mZombieHealthWaveStart);
                }
                if (mApp.IsLittleTroubleLevel() || mApp.mGameMode == GameMode.ChallengeColumn || mApp.IsLastStandLevel())
                {
        ...
        {
            if (StageIsNight() || (mApp.IsAdventureMode() && mLevel == ExtGameLevel.CUSTOM_LEVEL_CTCJ))
            {
                return;
            }
            if (HasLevelAwardDropped())
        ...
        public bool RowCanHaveZombieType(int theRow, ZombieType theZombieType)
        {
            ...
            return (mPlantRow[theRow] != PlantRowType.Pool || num >= 5 || Board.IsZombieTypePoolOnly(theZombieType)) && (mPlantRow[theRow] == PlantRowType.Pool || !Board.IsZombieTypePoolOnly(theZombieType)) && (theZombieType != ZombieType.Bobsled || mIceTimer[theRow] > 0) && (theZombieType != ZombieType.Dancer || StageHasPool() || (RowCanHaveZombies(theRow - 1) && RowCanHaveZombies(theRow + 1)));
        }
        ...
        public void PickZombieWaves()
        {
            ...
            for (int wave = 0; wave < mNumWaves; wave++)
            {
                ...
                if (mApp.IsLastStandLevel())
                {
                    zombiePicker.mZombiePoints = 4 * ((mChallenge.mSurvivalStage * GetNumWavesPerSurvivalStage() + wave + 10) * 2 / 5 + 1);
                }
        ...
        public void DrawUITop(Graphics g)
        {
            ...
            if (mStoreButton != null && mApp.IsLastStandLevel())
            {
                mStoreButton.Draw(g);
            }
        ...
        public void PickBackground()
        {
            ...
            case GameMode.Quickplay50:
                if (mLevel <= 1 * GameConstants.LEVELS_PER_AREA || mLevel == ExtGameLevel.CUSTOM_LEVEL_LLFX)
                {
                    mBackground = BackgroundType.Num1Day;
                }
                else if (mLevel <= 2 * GameConstants.LEVELS_PER_AREA || mLevel == ExtGameLevel.CUSTOM_XJZY || mLevel == ExtGameLevel.CUSTOM_MINIGAME_65 || mLevel == ExtGameLevel.CUSTOM_LEVEL_MYPD)
                {
                ...
                else if (mLevel <= 60 || mLevel == ExtGameLevel.CUSTOM_LEVEL_JJDCG)
                {
            ...
            if (StageHasGraveStones())
            {
                ...
                {
                    mChallenge.WhackAZombiePlaceGraves(30);
                }
                else if (mBackground == BackgroundType.Num2Night)
                {
        ...
        public void InitZombieWavesForLevel(int aForLevel)
        {
            ...
            if (mLevel == ExtGameLevel.CUSTOM_MINIGAME_55)
            {
                for (ZombieType i = 0; i < ZombieType.ZombieTypesCount; i++)
                {
                    if (ExtGameDef.CUSTOM_MINIGAME_55_ZOMBIE_WEIGHTS_OVERRIDE.ContainsKey(i))
                    {
                        mZombieAllowed[(int)i] = true;
                    }
                    else
                    {
                        mZombieAllowed[(int)i] = false;
                    }
                }
            }
        }
        ...
        public bool PlantingRequirementsMet(SeedType theSeedType)
        {
            ...
            return (theSeedType != SeedType.Cattail || CountEmptyPotsOrLilies(SeedType.Lilypad) != 0 || CountEmptyPotsOrLilies(SeedType.Flowerpot) != 0); ;
        }
        ...
        {
            return (mApp.IsFinalBossLevel() && mLevel != 60) || mApp.IsMiniBossLevel() || mApp.IsShovelLevel() || mApp.IsWallnutBowlingLevel() || mApp.IsLittleTroubleLevel() || (mApp.IsStormyNightLevel() && !(mApp.IsAdventureMode() && mLevel == ExtGameLevel.CUSTOM_XJZY)) || mApp.IsBungeeBlitzLevel() || mApp.mGameMode == GameMode.ChallengePortalCombat || mApp.mGameMode == GameMode.ChallengeColumn || mApp.mGameMode == GameMode.ChallengeInvisighoul || mLevel == ExtGameLevel.CUSTOM_MINIGAME_55;
        }
        ...
        {
            return mApp.IsLastStandLevel() && mChallenge.mSurvivalStage == GameConstants.LAST_STAND_FLAGS - 1;
        }
